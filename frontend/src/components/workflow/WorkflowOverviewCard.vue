...
<template>
  <Card>
    ...
    <CardContent v-if="instance" class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      <div>
        <div class="text-sm text-muted-foreground">标题</div>
        <div class="mt-1 font-medium">{{ instance.title }}</div>
      </div>
      <div>
        <div class="text-sm text-muted-foreground">流程定义</div>
        <div class="mt-1 font-medium">{{ instance.definitionName }}</div>
      </div>
      <div>
      ...
      <div>
        ...
        <div class="mt-1 font-medium">{{ instance.currentNodeName || '-' }}</div>
      </div>
      <div>
        <div class="text-sm text-muted-foreground">发起人</div>
        <div class="mt-1">{{ instance.userName || '-' }}</div>
      </div>
      <div>
        <div class="text-sm text-muted-foreground">提交时间</div>
        <div class="mt-1">{{ formatDateTime(instance.submitTime || instance.createTime) }}</div>
      </div>
      <div class="md:col-span-2 xl:col-span-2">
        <div class="text-sm text-muted-foreground">备注</div>
        <div class="mt-1 whitespace-pre-wrap">{{ instance.remark || '无备注' }}</div>
      </div>
    </CardContent>
