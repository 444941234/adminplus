  ...
  <Dialog :open="open" @update:open="(value) => emit('update:open', value)">
    <DialogContent class="sm:max-w-2xl max-h-[90vh] flex flex-col">
      ...
      <div class="flex-1 overflow-y-auto space-y-4 p-1 -m-1">
        ...
        <div class="space-y-2">
          <Label>流程标题</Label>
          <Input :model-value="title" placeholder="例如：费用报销申请" @update:model-value="handleTitleChange" />
        </div>
        <div class="space-y-2">
          <Label>
            申请表单
            <span v-if="definitionLoading" class="ml-2 text-xs text-muted-foreground">加载中...</span>
          </Label>
