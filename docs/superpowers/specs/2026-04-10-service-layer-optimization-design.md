...
### 4.1 XssSafeConverter 基类

**新文件位置**：`backend/src/main/java/com/adminplus/common/converter/XssSafeConverter.java`
...
/**
 * XSS 安全转换器基类
 * 在转换前自动对字符串字段进行 XSS 处理
 */
public abstract class XssSafeConverter<S, T> implements Converter<S, T> {
    
    // 不需要 XSS 处理的字段名列表（可被子类覆盖）
    protected List<String> skipXssFields() {
        return List.of("password", "username", "id", "code", "key", "token");
    }
    ...
    public final T convert(S source) {
        ...
        S sanitizedSource = sanitizeSource(source);
        return doConvert(sanitizedSource);
    }
    ...
    /**
     * XSS 清洗源对象
     * 默认处理所有 String 字段，跳过 skipXssFields() 中列出的字段
     */
    protected S sanitizeSource(S source) {
        if (source == null) {
            return null;
        }
        
        Class<?> clazz = source.getClass();
        List<String> skipFields = skipXssFields();
        
        // 处理 Record 类型（Request DTO 通常为 record）
        if (clazz.isRecord()) {
            return sanitizeRecord(source, clazz, skipFields);
        }
...
| **步骤四** | 统一 XSS 处理 | 新增约15 Converter，修改6服务 | 可在步骤三完成后执行 |
...
| 4 | `common/converter/XssSafeConverter.java` | XSS 安全转换器基类 |
| 4 | `common/converter/UserCreateRequestToEntityConverter.java` | 用户创建 |
| 4 | `common/converter/UserUpdateRequestToEntityConverter.java` | 用户更新 |
| 4 | `common/converter/RoleCreateRequestToEntityConverter.java` | 角色创建 |
| 4 | `common/converter/RoleUpdateRequestToEntityConverter.java` | 角色更新 |
| 4 | `common/converter/DeptCreateRequestToEntityConverter.java` | 部门创建 |
| 4 | `common/converter/DeptUpdateRequestToEntityConverter.java` | 部门更新 |
| 4 | `common/converter/MenuCreateRequestToEntityConverter.java` | 菜单创建 |
| 4 | `common/converter/MenuUpdateRequestToEntityConverter.java` | 菜单更新 |
| 4 | `common/converter/ConfigCreateRequestToEntityConverter.java` | 配置创建 |
| 4 | `common/converter/ConfigUpdateRequestToEntityConverter.java` | 配置更新 |
| 4 | `common/converter/WorkflowStartRequestToEntityConverter.java` | 工作流发起 |
...
| 1-4 | `service/impl/UserServiceImpl.java` | 使用新工具类 |
| 1-4 | `service/impl/RoleServiceImpl.java` | 使用新工具类 |
| 1-4 | `service/impl/DeptServiceImpl.java` | 使用新工具类 |
| 1-4 | `service/impl/MenuServiceImpl.java` | 使用新工具类 |
| 1-4 | `service/impl/FileServiceImpl.java` | 使用新工具类 |
| 1-4 | `service/impl/ConfigServiceImpl.java` | 使用新工具类 |
| 2 | `service/impl/WorkflowInstanceServiceImpl.java` | 重构为协调者 |
| 3 | `enums/WorkflowStatus.java` | 新增状态行为方法 |
| 4 | `common/config/ConverterConfig.java` | 注册 Converter |