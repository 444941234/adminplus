...
| 测试文件 | 覆盖功能 |
|----------|----------|
| `useCRUD.test.ts` | CRUD页面模式封装、列表获取、对话框状态、表单管理、增删改操作 |
| `usePermission.test.ts` | 权限检查、角色检查、超级管理员判断、权限过滤器 |
...
| 测试文件 | 覆盖功能 |
|----------|----------|
| `validators.test.ts` | 邮箱验证、手机号验证、密码强度校验、角色代码验证 |
| `format.test.ts` | 日期时间格式化 |
| `activityUtils.test.ts` | 活动数据工具函数 |

### 工作流组合式函数测试

| 测试文件 | 覆盖功能 |
|----------|----------|
| `useWorkflowStatus.test.ts` | 流程状态标签、样式变体、终态判断、操作权限判断 |
| `useWorkflowActions.test.ts` | 工作流动作封装、撤回、取消、催办、审批、驳回、回退、加签、loading状态管理 |
...
| `config.test.ts` | 配置分组API、配置项API、导入导出、缓存刷新、历史回滚 |
