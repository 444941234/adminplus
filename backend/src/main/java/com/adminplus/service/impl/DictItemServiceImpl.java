...
public class DictItemServiceImpl implements DictItemService {
    ...
    public List<DictItemResponse> getDictItemsByDictId(String dictId) {
    ...
    public DictItemResponse getDictItemById(String id) {
    ...
    public DictItemResponse createDictItem(DictItemCreateRequest request) {
        DictEntity dict = dictRepository.findById(request.dictId())
                .orElseThrow(() -> new BizException("字典不存在"));
        ...
        item.setStatus(request.status() != null ? request.status() : 1);
    ...
    @Transactional
    @CacheEvict(value = "dictItem", allEntries = true)
    public DictItemResponse updateDictItem(String id, DictItemUpdateRequest request) {
        DictItemEntity item = dictItemRepository.findById(id)
                .orElseThrow(() -> new BizException("字典项不存在"));
        ...
        if (request.getParentId().isPresent()) {
            String newParentId = request.getParentId().get();
        ...
        request.getLabel().ifPresent(item::setLabel);
        request.getValue().ifPresent(item::setValue);
        request.getSortOrder().ifPresent(item::setSortOrder);
        request.getStatus().ifPresent(item::setStatus);
     ...
     */
    private DictItemResponse createWithChildren(DictItemResponse original, List<DictItemResponse> children) {
